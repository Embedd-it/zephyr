...
/**
 * @brief User callback, registered by @copybrief usbh_vendor_interrupt_subscribe
 *
 * @param dev           Pointer to user context
 ...
 *
 */
typedef int (*usbh_vendor_interrupt_write_t)(struct device const *dev, uint8_t ep_id,
 ...
 *
 */
typedef int (*usbh_vendor_interrupt_subscribe_t)(struct device const *dev, uint8_t ep_id,
 ...
 *
 */
typedef int (*usbh_vendor_interrupt_unsubscribe_t)(struct device const *dev, uint8_t ep_id);
...
__subsystem struct usbh_vendor_driver_api {
	...
	/**
	 * @driver_ops_mandatory @copybrief usbh_vendor_interrupt_subscribe
	 */
	usbh_vendor_interrupt_subscribe_t interrupt_subscribe;

	/**
	 * @driver_ops_mandatory @copybrief usbh_vendor_interrupt_unsubscribe
	 */
	usbh_vendor_interrupt_unsubscribe_t interrupt_unsubscribe;

	/**
	 * @driver_ops_mandatory @copybrief usbh_vendor_interrupt_write
	 */
 ...
 * @return 0 on success, negative errno value on failure.
 */
__syscall int usbh_vendor_interrupt_subscribe(struct device const *dev, uint8_t ep_id,
					      usbh_vendor_callback_t cb, void *user_ctx);

static inline int z_impl_usbh_vendor_interrupt_subscribe(struct device const *dev, uint8_t ep_id,
							 usbh_vendor_callback_t cb, void *user_ctx)
{
	struct usbh_vendor_driver_api const *api = DEVICE_API_GET(usbh_vendor, dev);
 ...
 * @return 0 on success, negative errno value on failure.
 */
__syscall int usbh_vendor_interrupt_unsubscribe(struct device const *dev, uint8_t ep_id);

static inline int z_impl_usbh_vendor_interrupt_unsubscribe(struct device const *dev, uint8_t ep_id)
{
	struct usbh_vendor_driver_api const *api = DEVICE_API_GET(usbh_vendor, dev);
 ...
 * @return 0 on success, negative errno value on failure.
 */
__syscall int usbh_vendor_interrupt_write(struct device const *dev, uint8_t ep_id,
					  const uint8_t *const buf, const size_t buf_len);

static inline int z_impl_usbh_vendor_interrupt_write(struct device const *dev, uint8_t ep_id,
						     const uint8_t *const buf, const size_t buf_len)
{
	struct usbh_vendor_driver_api const *api = DEVICE_API_GET(usbh_vendor, dev);
