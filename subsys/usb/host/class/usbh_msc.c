...
struct lun_data {
	...
	/* Whether the drive is read-only or writeable */
	bool write_protect;
...
static int usbh_msc_disk_access_status(struct disk_info *disk)
{
...
static int usbh_msc_disk_access_read(struct disk_info *disk, uint8_t *data_buf,
...
static int usbh_msc_disk_access_write(struct disk_info *disk, const uint8_t *data_buf,
				      uint32_t start_sector, uint32_t num_sector)
{
	struct lun_data *lun_data = get_lun_data_from_disk(disk);
	struct usbh_msc_data *dev_data = lun_data->dev_data;
...
static int usbh_msc_disk_access_erase(struct disk_info *disk, uint32_t start_sector,
				      uint32_t num_sector)
{
	ARG_UNUSED(disk);
...
static int usbh_msc_disk_access_ioctl(struct disk_info *disk, uint8_t cmd, void *buff)
{
	struct lun_data *lun_data = get_lun_data_from_disk(disk);
	struct usbh_msc_data *dev_data = lun_data->dev_data;
	int ret = 0;
...
static int usbh_msc_disk_access_init(struct disk_info *disk)
{
	return usbh_msc_disk_access_ioctl(disk, DISK_IOCTL_CTRL_INIT, NULL);
}

/*
 * Disk access vtable
 */
static const struct disk_operations usbh_msc_disk_operations = {
	.init = usbh_msc_disk_access_init,
	.status = usbh_msc_disk_access_status,
	.read = usbh_msc_disk_access_read,
	.write = usbh_msc_disk_access_write,
	.erase = usbh_msc_disk_access_erase,
	.ioctl = usbh_msc_disk_access_ioctl,
};
			  ...
			  uint8_t iface)
{
	...
	for (size_t lun_index = 0; lun_index <= dev_data->max_logical_unit &&
		...
		lun_data->disk.ops = &usbh_msc_disk_operations;
...
#define USBH_DEVICE_DEFINE(index, _)                                                               \
	...
	DEVICE_DEFINE(usbh_msc_##index, "usbh_msc_" #index, NULL, NULL, &dev_data_##index,         \
		      &dev_config_##index, POST_KERNEL, CONFIG_USBH_MSC_INIT_PRIORITY, NULL);      \
                                                                                                   \
